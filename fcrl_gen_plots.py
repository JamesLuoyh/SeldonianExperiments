...
def vfae_example(
    ...
    data_fracs = [ 1]  # 0.1, 0.15,0.25,0.40,0.65,#0.001,0.01,0.05,0.1,0.33,0.66,1
    # adult
    batch_epoch_dict = {
    #   0.001:[30,1000],
    #   0.005:[50,1000],
    ...
    for epsilon in epsilons:
        specfile = os.path.join(
            spec_rootdir,
            f"fcrl_{dataset}_mutual_information_{epsilon}.pkl"
        )
        spec = load_pickle(specfile)
        results_dir = os.path.join(results_base_dir,
            f"fcrl_{dataset}_mutual_information_{epsilon}_{performance_metric}")
        plot_savename = os.path.join(
            results_dir, f"fcrl_{dataset}_mutual_information__{epsilon}_{performance_metric}.pdf"
        )
