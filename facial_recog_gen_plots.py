...
def vfae_example(
    ...
    for epsilon in epsilons:
        ...
        specfile = os.path.join(
            spec_rootdir,
            f"unsupervised_age_cnn_vfae{version_subs}_mutual_information_{epsilon}.pkl"
        )
        print(f"unsupervised_age_cnn_vfae{version_subs}_mutual_information_{epsilon}.pkl")
        spec = load_pickle(specfile)
        
        torch.manual_seed(2023) #spec.seed)
        np.random.seed(2023) #spec.seed)
        # batch_epoch_dict = spec.batch_epoch_dict
        results_dir = os.path.join(results_base_dir,
            f"cnn_vfae{version_subs}_mutual_information_{epsilon}_{performance_metric}")
        ...
        for i in range(2):
            ...
            plot_generator.make_plots(
                fontsize=12,
                legend_fontsize=8,
                performance_label=['auc', f'dp < {dp_constraint}'],
